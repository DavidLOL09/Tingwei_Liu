...
try:
    ...
    send2trash.send2trash(output_origin)
    os.makedirs(output_origin)
...
for e in e_range:
    for sin2 in sin2Val:
        ...
        cmd = f'python Stn51Sim_origin_backlope.py --output_filename {output_filename}_{e:.1f}-{e+0.1:.1f}eV_{sin2:.1f}sin2_{n_cores}cores --n_cores {n_cores} --min_energy {e:.1f} --max_energy {e+0.1:.1f} --sin2 {sin2:.1f} --num_icetop {num_icetop} --output_origin {output_origin} --output_filename {output_filename}'
