...
def avail_check():
    return 'File loaded'
def getTriggerRatePerBin(sim_folder, e_bins, sin2_bins, trigger_names):
    ...
    # sin2_bins                 : list of zenith bin edges, in radians
    ...
    for trigger in trigger_names:
        trig_rate_per_bin[trigger] = np.zeros((len(sin2_bins),len(e_bins)-1))
        n_trig_per_bin[trigger] = np.zeros((len(sin2_bins),len(e_bins)-1))

    ic(f'reading files in {sim_folder} with trigger names {trigger_names}')

    for iE in range(len(e_bins)-1):
        for iS in range(len(sin2_bins)-1):
            # Load files for given bin only
            nurFiles= []

            for file in os.listdir(sim_folder):
                # ic(f'checking if {e_bins[iE]:.1f}-{e_bins[iE+1]:.1f}eV_{sin2_bins[iS]:.1f}sin2 in file')
                if file.endswith('.nur') and (f'{e_bins[iE]:.1f}-{e_bins[iE+1]:.1f}eV_{sin2_bins[iS]:.1f}sin2' in file):
                    nurFiles.append(os.path.join(sim_folder, file))
                    ic(f'adding file {nurFiles[-1]}')
            if nurFiles == []:
                ic(f'checking if {e_bins[iE]:.1f}-{e_bins[iE+1]:.1f}eV_{sin2_bins[iS]:.1f}sin2 in file')
                ic(f'No files found in {sim_folder}, quitting')
                continue
                # quit()
...
def getParametersPerEvent(simulation_files_folder, trigger, output_path, max_distance, e_bins=None, zen_bins=None, rate_per_bin=None, n_trig_per_bin=None):
    eventWriter.begin(os.path.join(output_path,'IceTopSim_Weights.nur'))
    # Trigger should be a single string
